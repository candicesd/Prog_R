...
sd(df$SHOT_CLOCK, na.rm = TRUE)
     ...
     #les quartiles
     quantile(df$SHOT_CLOCK, probs = seq( 0.25, 1,  0.25), na.rm = TRUE)
     #les déciles
     quantile(df$CLOSE_DEF_DIST, probs = seq(0.1,1,0.1),na.rm = TRUE)
...
df_total_tri <- order(df_total$PTS_MARQUES)
